...
- **필요한 API · 데이터**에는 이미 있는 것과 새로 필요한 것을 나눠 적는다. 있는지는 백엔드 코드로 확인한다. 경로가 클래스의 `@RequestMapping`과 합쳐지는 경우가 많아서 끝부분으로 찾는다. 파라미터(정렬값 등)도 코드에서 확인한다.
  ```bash
  gh search code '"/posts"' --repo NAR-GG/nar-back-repo --json path --jq '.[].path'
  ```
  모바일 `docs/api-reference.md`는 `@Hidden` 엔드포인트가 빠져 있고 수동으로 갱신돼서 기준으로 쓰지 않는다. 코드에서도 못 찾으면 추측하지 말고 "확인 필요"라고 쓴다. 목업이나 다른 문서에 적힌 경로를 그대로 옮기지 않는다.
